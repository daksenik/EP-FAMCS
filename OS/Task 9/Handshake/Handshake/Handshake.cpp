...
DWORD WINAPI ServerThread(PVOID pvParam) {
  ...
  fShutdown = FALSE;
   ...
   while (!fShutdown) {
      ...
      if (!fShutdown)_tcsrev(g_szSharedRequestAndResultBuffer);// Process the client's request (reverse the string)
...
{
		 DWORD exitCode;
		 GetExitCodeThread(hThreadServer, &exitCode);
...
void Dlg_OnCommand(HWND hwnd, int id, HWND hwndCtl, UINT codeNotify) {
   ...
   switch (id) {
      ...
      case IDC_SUBMIT:  // Submit a request to the server thread
		  GetExitCodeThread(hThreadServer, &exitCode);//daksenik
		  if (exitCode != STILL_ACTIVE)
		  {
			  EnableWindow(GetDlgItem(hwnd, IDC_SUBMIT), false);
		  }
         ...
         Edit_GetText(GetDlgItem(hwnd, IDC_REQUEST), //Look for it in Windowsx.h
            ...
            sizeof(g_szSharedRequestAndResultBuffer));//последний параметр вроде как должен быть другим, но так работает
