...
bool bSuspendE,bSuspendR,bSuspendL;
...
void TerminateLine(HWND hWnd, HMENU hMenu, BOOL *fTerminateX);
...
//As it is named it is possible to verify if there is only one application instance too!
	...
	//if(!(hMutex=CreateMutex(NULL,FALSE,szMutexName))){//FALSE sets it In SIGNALED STATE! NO OWNERSHIP	
	...
	MessageBox(NULL,msgHINST,"Four Threads",MB_OK);
...
//	int wmId, wmEvent;
	switch (message) 
	{//redirecting of messages to other functions
...
LONG WndProc_OnCommand (HWND hWnd,int id,HWND hwndCtl,UINT codeNotify)
{
		...
		switch (id)
		{
			...
			case IDM_CH_THR_PR_ELL:
				DialogBox(hInst, (LPCTSTR)IDD_DIALOG_PRIORITY, hWnd,(DLGPROC)ChangeThreadPriority_Box);
				return 0L;
...
LONG WndProc_OnCreate(HWND hWnd,LPCREATESTRUCT lpCreateStruct)
{
	...
	if(!hThreadE[0])
	{
		MessageBox(NULL,"Thread start Error","PaintEllipse Thread",MB_OK|MB_ICONEXCLAMATION);
		return FALSE;
	}

    hThreadE[1]=(HANDLE)_beginthreadex(NULL,0,PaintRectangle,(void *)hWnd,ususpend,&thridE);
	if(!hThreadE[1])
	{
		MessageBox(NULL,"Thread start Error","PaintRectangle Thread",MB_OK|MB_ICONEXCLAMATION);
		return FALSE;
	}

	hThreadE[2] = (HANDLE)_beginthreadex(NULL, 0, PaintLine, (void *)hWnd, ususpend, &thridE);
	if (!hThreadE)
	{
		MessageBox(NULL, "Thread start Error", "PaintLine Thread", MB_OK | MB_ICONEXCLAMATION);
		return FALSE;
	}
...
LONG WndProc_OnDestroy(HWND hWnd)
{
	...
	if(bSuspendE)ResumeThread(hThreadE[0]);
	if(bSuspendR)ResumeThread(hThreadE[1]);
	if(bSuspendL)ResumeThread(hThreadE[2]);
	...
	{
		MessageBox(NULL,"CloseHandle  failed","PaintEllipse Thread",MB_OK|MB_ICONEXCLAMATION);
	}
	...
	{
		MessageBox(NULL, "CloseHandle  failed", "PaintLine Thread", MB_OK | MB_ICONEXCLAMATION);
	};
...
{ 
	...
	int r = MessageBox(hWnd,sznWM,"Primary Thread WM_PAINT count",MB_OKCANCEL|MB_ICONEXCLAMATION);
...
unsigned int __stdcall  PaintLine(void *hWnd) {
	...
	srand((unsigned int)hWnd + 100);
	while (!fTerminateL) {	// Is not it to be continued?
							// To be continued!
		EnterCriticalSection(&criticalSection);
		hDC = GetDC((HWND)hWnd);
		nRed = rand() % 255; nGreen = rand() % 255; nBlue = rand() % 255;
		...
		hBrush = CreateSolidBrush(RGB(nRed, nGreen, nBlue));
		hOldBrush = (HBRUSH)SelectObject(hDC, hBrush);
		MoveToEx(hDC, min(xLeft, xRight), min(yTop, yBottom), NULL);
		LineTo(hDC, max(xLeft, xRight), max(yTop, yBottom));
		SelectObject(hDC, hOldBrush);
		DeleteObject(hBrush);
		ReleaseDC((HWND)hWnd, hDC);
		LeaveCriticalSection(&criticalSection);
...
void TerminateEllipse(HWND hWnd,HMENU hMenu,BOOL *fTerminateE)
{
	...
	else
	{
		...
		*fTerminateE = false;
		...
		if (!hThreadE[0])MessageBox(NULL, "Thread start Error", "PaintEllipse Thread", MB_OK | MB_ICONEXCLAMATION);
	}
	return;
}
...
void TerminateRectangle(HWND hWnd, HMENU hMenu, BOOL *fTerminateR)
{
	if (!*fTerminateR)
	{
		...
		*fTerminateR = true;
		...
		if (!CloseHandle(hThreadE[1]))
		{
			MessageBox(NULL, "CloseHandle  failed", "PaintRectangle Thread", MB_OK | MB_ICONEXCLAMATION);
		};
	}
	else
	{
		...
		*fTerminateR = false;
		...
		if (!hThreadE[1])MessageBox(NULL, "Thread start Error", "PaintRectangle Thread", MB_OK | MB_ICONEXCLAMATION);
	}
	return;
}
...
void TerminateLine(HWND hWnd, HMENU hMenu, BOOL *fTerminateL)
{
	if (!*fTerminateL)
	{
		...
		*fTerminateL = true;
		if (bSuspendL)ResumeThread(hThreadE[2]);
		WaitForSingleObject(hThreadE[2], INFINITE);//Block the primary thread if PaintRectangle
												   // is in the suspended state
		if (!CloseHandle(hThreadE[2]))
		{
			MessageBox(NULL, "CloseHandle  failed", "PaintLine Thread", MB_OK | MB_ICONEXCLAMATION);
	...
	else
	{
		...
		*fTerminateL = false;
		...
		hThreadE[2] = (HANDLE)_beginthreadex(NULL,0,PaintLine,(void *)hWnd,ususpend,&thridE);
		if (!hThreadE[2])MessageBox(NULL, "Thread start Error", "PaintLine Thread", MB_OK | MB_ICONEXCLAMATION);
	}
	return;
}
