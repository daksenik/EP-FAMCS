...
public class DBRequests {
    ...
    private static final String CHECK_ADMIN = "select is_admin from users where LOGIN = ? and PASSWORD = ?";
    ...
    public static int checkLogin(String login, String password) {
        Connection cn = db.getConnection();
        try(PreparedStatement update = cn.prepareStatement(CHECK_ADMIN)){
    ...
    public static void updatePassword(String login, String newPassword) {
        Connection cn = db.getConnection();
        try(PreparedStatement update = cn.prepareStatement(UPDATE_PASSWORD)) {
